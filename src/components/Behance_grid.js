...
const BehanceItems = () => {
  const [items, setItems] = useState(behanceItem);

  const handleLike = (index) => {
    const newItems = [...items];
    newItems[index].feLike = parseInt(newItems[index].feLike) + 1;
    setItems(newItems);
  };
      ...
      <div className="container-fluid px-4 py-4 border-b">
        <div className="grid md:grid-cols-2 sm:grid-cols-1 lg:grid-cols-3 gap-4">
          {items.map((item, index) => (
            <div key={index} className="category-items cursor-pointer">
              ...
              <div className="info flex justify-between">
                ...
                <div className="count-data flex pr-1 pb-2">
                  <div className="likes flex items-center px-2" onClick={() => handleLike(index)}>
